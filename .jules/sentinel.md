...
**Prevention:** To prevent XSS or HTML injection when rendering untrusted markdown in Qt, ALWAYS apply the `QTextDocument::MarkdownNoHTML` feature flag. Note: When combining flags via bitwise OR (e.g., `QTextDocument::MarkdownDialectGitHub | QTextDocument::MarkdownNoHTML`), you must explicitly cast them using `QTextDocument::MarkdownFeatures(...)` to prevent invalid integer conversion errors during compilation.
